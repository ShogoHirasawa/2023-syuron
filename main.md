...
本研究の目的は洪水被害発生時におけるオフライン環境において、避難者自身のスマートフォンのブラウザアプリケーションを通じて避難ルートを検討するための情報が掲載された洪水ハザードマップを閲覧可能とするシステムの提案することである。本目的を達成するためのシステム構築について「3. 提案手法」で述べた。本章では構築したシステムが、洪水災害時を想定したオフライン環境下において、本研究の目的を満たすかどうかを検証する。本検証は以下の2つに分けて行われる。

- 検証 1:オフライン環境下で適切に Web ハザードマップを避難者に提供できるシステムであることの検証
- 検証 2:作成した洪水ハザードマップを通して避難者が危険地帯を把握できることの検証

検証 1 では、オフライン環境において本システムの稼働可能時間とハザードマップ表示までかかる描画時間を検証する。避難時という切迫した環境のなかで、稼働時間が短い、加えてハザードマップの表示に時間を要するようでは、システムの可用性を満たしているとは言えない。避難想定時間を上回る稼働時間と、従来のスマホ操作と同様かそれ以上の表示スピードでハザードマップが閲覧できることが求められる。これを検証するために、内閣府が定義する洪水災害時の避難予想時間（17時間）を閾値に設定し、これを上回る稼働時間が確保できるかを検証した。また、Google 社が定義する、ページの読み込みパフォーマンス、インタラクティブ性等を測る指標である Core Web Vitals を用いて、本システムにおけるハザードマップの表示スピードを計測した。調査 2 では参加者を集め、本システムで実装した標高の３D表示が、洪水災害時の危険地帯である低地と傾斜地を参加者に訴求できているかをインタビュー形式で検証した。参加者を10人集め、標高を2Dで表示したハザードマップと３Dで表示したハザードマップを見せ、3D表示の効果を分析した。検証 1 では避難想定時間を超えるシステム稼働時間が確保できることがわかるとともに、Core Web Vitalsにおいて、利用者にストレスのかからない Web ページ描画が可能であることが計測できた。調査 2 では洪水災害において危険地帯とされる、浸水予想域、低地、傾斜地を認識させることが可能であることが示された。本章では検証1を「4.1. システム稼働時間とページ表示スピードの計測」で、検証2を「4.2. 3D標高表現に関するユーザビリティ評価」で論じる。
