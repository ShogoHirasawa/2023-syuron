...
ハザードマップを保管している場合、発災時にそれを取り出し避難を検討することができる。一方、ハザードマップを保管していない場合、避難者は自治体の Web ページにアクセスしハザードマップを閲覧するフローが想定される。金井ほか（2017）はハザードマップの保有世帯を 6.9%と推定している。これを踏まえると過半数以上の世帯は発災時にハザードマップがない状態となることが予想される。数多くの世帯がハザードマップを保有していないことを踏まえ、インターネット障害が起きておらず接続で可能な状態であっても、一度に大勢が被災地帯のハザードマップにアクセスしようとすると、サーバー側のキャパシティを超え、アクセスが難しい状態となる。2024 年 1 月 1 日に発生した令和 6 年能登半島地震の被災地である輪島市と珠洲市の津波ハザードマップにおいて、発災直後にはサイトにアクセスできない状態が発生した。筆者は発災直後に CheckIfDown と呼ばれるサービスを用いてこれを検証した。CheckIfDown は特定のウェブサイトのアクセシビリティを判定するために設計されたツールであり、サーバー側に障害が発生しているかを確認することができる。輪島市が公開する津波ハザードマップに対し、1 月 1 日(月) 16:50 頃アクセスを試みたがサーバーがダウンしている表示がでた（図 31）。また、珠洲市も同様に、Web 上で公開される津波ハザードマップに対して 1 月 1 日(月) 16:58 頃アクセスしたが、ページを表示することはできなかった（図 32）。上記の結果を踏まえると、インターネット障害が起きていなくとも、ハザードマップにアクセスできない状況は発生しうることが予想される。紙媒体のハザードマップ

（図 31 CheckIfDown にて輪島市 津波ハザードマップにアクセスした時の分析結果）
（図 32 CheckIfDown にて珠洲市 津波ハザードマップにアクセスした時の分析結果）

- データをオフラインで集約することで、紙データで入力していた情報を GIS データとして扱える様になる
