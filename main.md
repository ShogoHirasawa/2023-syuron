...
（図28 どのような頻度でデジタル地図を利用しますか？に対する回答）
（図29 どのような頻度でハザードマップを閲覧しますか？に対する回答）
...
本節では、文京区周辺の2D の標高表現と陰影起伏を加えたハザードマップと同じく文京区周辺の3D の標高表現と陰影起伏のハザードマップを見比べて得られた参加者の回答を示す。はじめに、2D の標高表現と陰影起伏を加えたハザードマップを閲覧してどの地域を危険だと認識したかを示し、その後3Dハザードマップではどこを認識したかを述べる。最後に、2D と3Dを見比べた上で得られた回答をまとめる。まず、2D の標高表現と陰影起伏を加えたハザードマップを閲覧した上で得られた回答を表4に示す。結果を見ると、2Dのハザードマップでは浸水予測域が広がっている場所と浸水予想深度の深い場所を危険地帯だと認識している回答が多く見られ、低地や傾斜地を危険地帯だと認識している回答は少なかった。

（表4 2Dマップを見た上でどこを危険地帯だと感じたか）
（*ここって具体的に地名を回答していた場合、その場所を地図にプロットした図を用意するべき？）
