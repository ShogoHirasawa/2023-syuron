...
検証 1 では、オフライン環境において本システムの稼働可能時間とハザードマップ表示までかかる描画時間を検証した。調査 2 では参加者を集め、作成したハザードマップが洪水災害時における危険地帯の認識につながるかを検証した。検証 1 では避難想定時間を超えるシステム稼働時間が確保できることが確かめられると同時に、適切なスピードでWeb ページ描画を表示できることが示された。調査 2 本研究で作成したハザードマップが浸水予想域、低地、傾斜地を利用者に認識させることが可能であることが示された。これにより本目的は達成された。
